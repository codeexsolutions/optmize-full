 ...
 * este código rodar dentro de um Web Worker, ser testado sem navegador, e
 * atravessar a migração para o React sem ser tocado.
 *
 * ---------------------------------------------------------------------------
 * SOBRE O PORTE
 * ---------------------------------------------------------------------------
 * Veio de `public/geometria.js`. Nenhuma conta mudou — o que entrou foram os
 * tipos e o `export`; o que saiu foi a dependência do escopo global, que era
 * como os arquivos se enxergavam antes. É a receita para os outros módulos de
 * domínio (nfp, encaixe-motor, moldes, vetor): **portar, não reescrever**.
 */
 ...
 */
export function areaComSinal(pontos: readonly Ponto[]): number {
  let soma = 0;
...
export function caixaDeContorno(pontos: readonly Ponto[]): Caixa {
 ...
 */
export function ladoMenorDoContorno(pontos: readonly Ponto[]): number {
 ...
 */
export function simplificar(pontos: readonly Ponto[], tolerancia: number): Ponto[] {
