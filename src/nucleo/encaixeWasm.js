/**
 * ===========================================================================
 * WASM — o laço quente do encaixe por contorno, em WebAssembly
 * ===========================================================================
 *
 * O caminho que roda de verdade: 3,9x mais rápido que o JavaScript. Quando ele
 * não está disponível — módulo não carregado, tecido largo demais para o plano
 * de memória — devolve `null` e o motor segue pelo caminho de sempre, que
 * continua sendo a referência de correção.
 *
 * **A QUEDA PARA O CAMINHO LENTO É SILENCIOSA POR DESENHO.** Foi assim que uma
 * ferramenta que compilava e não prestava passou despercebida: o `cargo` lia o
 * `.cargo/config.toml` da pasta errada, o módulo saía sem o símbolo, e o
 * encaixe voltava para o JavaScript sem ninguém ver. Quem confere isso é a
 * bancada, e é por isso que ela existe.
 *
 * ---------------------------------------------------------------------------
 * O CICLO COM O MOTOR
 * ---------------------------------------------------------------------------
 *
 * Este arquivo importa do `encaixeMotor` e o `encaixeMotor` importa deste. Em
 * `<script>` global isso era invisível — os dois dividiam o mesmo escopo. Em
 * ESM é um ciclo declarado, e funciona porque os dois lados só se chamam em
 * TEMPO DE EXECUÇÃO, e `function` é içada: quando a primeira chamada acontece,
 * os dois módulos já terminaram de avaliar.
 */
...
export function garantirMemoria(bytes) {
 ...
 */
export function prepararUnidadesNoWasm(unidades) {
