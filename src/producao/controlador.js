...
/*
 * Aqui morava a TELA DE MOLDES, em ~1.400 linhas imperativas: a estante, o
 * passo a passo de criar molde e o painel de arte, com sete variáveis de
 * módulo guardando o que estava na tela e um `renderX()` chamado à mão depois
 * de cada mexida. Ela foi para o React:
 *
 *   `src/telas/Moldes.tsx`                    a estante
 *   `src/telas/moldes/EditorDeMolde.tsx`      o passo a passo
 *   `src/telas/moldes/EnvioParaEncaixe.tsx`   a arte, a prévia e o envio
 *   `src/telas/moldes/vocabulario.ts`         os tipos, os papéis e as frases
 *   `src/api/moldes.ts`                       a conversa com o servidor
 *
 * Duas coisas desceram para lugares compartilhados no caminho, porque o
 * Encaixe — que continua aqui — também as usa: `motores/nomeDeArquivo.js` (o
 * "5x" no nome do arquivo) e `utils/coresDePeca.ts` (as dez cores das peças).
 *
 * O que ficou deste lado é a ponte `mandarMolde`, no objeto de controle.
 */

/*
 * Aqui morava a TELA DE PROJETOS, em 620 linhas imperativas: `innerHTML` para
 * a estante, `getElementById` para cada campo do editor, e uma lista de peças
 * de módulo que o DOM e o objeto tentavam manter iguais. Ela foi para o React:
 * `src/telas/Projetos.tsx` tem a tela e o estado, `src/api/projetos.ts` tem a
 * conversa com o servidor, e `medidasDoArquivo` desceu para
 * `src/motores/medidaDoArquivo.js`, onde já morava a leitura irmã dela.
 *
 * O que ficou deste lado é a ponte `mandarProjeto` (lá embaixo, no objeto de
 * controle): o Encaixe ainda é imperativo, então quem leva um projeto até ele
...
return {
 ...
 /*
  * A ponte com a tela de Projetos, que é React.
  *
  * Ela manda o projeto inteiro: os ajustes guardados vão para os campos do
  * Encaixe e as artes entram na lista, já sem fundo. Os AJUSTES VÃO ANTES das
  * peças, e não é detalhe — o giro de cada peça é lido do seletor geral no
  * momento em que ela entra, então inverter a ordem faria o projeto voltar com
  * o giro da última vez em vez do que ele guarda.
  *
  ...
  *
  * As peças chegam com a arte JÁ DESENHADA dentro do contorno (o `desenho` de
  * cada uma), no dpi que a pessoa escolheu. Desenhar é caro e é trabalho de
  * tela — quem tem a arte na mão e o ajuste aberto é aquela tela; aqui só
 ...
 async mandarProjeto({ nome, pecas, unidades, ajustes }) {
   ...
   escrever(encaixeLarguraInput, ajustes.larguraTecido);
   escrever(encaixeEspacoInput, ajustes.espaco);
   escrever(encaixeEspacoYInput, ajustes.espaco);
   escrever(encaixeComprimentoInput, ajustes.comprimentoBancada);
   if (encaixeGiroTodasSelect && ajustes.giro) encaixeGiroTodasSelect.value = ajustes.giro;

   /*
    * Uma pausa curta para a aba do Encaixe aparecer antes de o trabalho pesado
