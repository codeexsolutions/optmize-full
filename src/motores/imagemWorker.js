 ...
 * ===========================================================================
 *
 * Veio de `public/imagem-worker.js` por transformação mecânica. **Nenhuma
 * conta mudou** — o que mudou foi de onde os arquivos da IA são carregados:
 *
 *   antes:  "/ia/ort.webgpu.bundle.min.mjs"   (caminho absoluto)
 *   agora:  `${import.meta.env.BASE_URL}ia/…` (relativo à base do app)
 *
 * O absoluto funcionava porque o Express publica `estatico/` na raiz. Mas em
 * desenvolvimento o Vite atende noutra porta e serve `estatico/` debaixo da
 * base (`/app/`), então `/ia/…` daria 404 e a rede simplesmente não carregaria
 * — em silêncio, porque o `catch` do carregamento derruba para a ampliação
 * limpa. Com a base, os dois casos funcionam.
 *
 * O resto do cabeçalho original continua abaixo, porque explica as decisões
 * que ainda valem: por que ladrilho, por que margem, e por que a cor é tratada
 * do jeito que é.
 */
...
/** A raiz de onde a IA é servida. Ver o cabeçalho: muda entre Vite e Express. */
const RAIZ_DA_IA = `${import.meta.env.BASE_URL}ia/`;

const CAMINHO_ORT = `${RAIZ_DA_IA}ort.webgpu.bundle.min.mjs`;
...
async function ligar(avisar, qual) {
  ...
  if (!ort) {
    ...
    ort.env.wasm.wasmPaths = RAIZ_DA_IA;
  ...
  for (const provedor of tentativas) {
    try {
      ...
      sessao = await ort.InferenceSession.create(RAIZ_DA_IA + MODELOS[escolhido].arquivo, {
