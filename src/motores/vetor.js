 ...
 * ===========================================================================
 *
 * Recebe os pixels de uma imagem e devolve contornos: primeiro as regiões de
 * cor, depois a borda de cada uma, depois a curva que passa por essa borda.
 *
 * ---------------------------------------------------------------------------
 * POR QUE ESTE ARQUIVO É `.js` NUMA PASTA DE `.ts`
 * ---------------------------------------------------------------------------
 *
 * Ele veio de `public/vetor.js`, e **nenhuma conta mudou**. Entraram o
 * `import` do que antes vinha do escopo global e o `export` do que a tela e o
 * worker chamam; e mais nada.
 *
 * Tipar isto à mão seriam ~180 anotações em 1.200 linhas de laço sobre pixel,
 * e cada uma é uma chance de ler um índice errado e mudar um resultado sem
 * ninguém ver — que é exatamente o que a regra do projeto proíbe ("se uma
 * conta mudou de resultado, o porte está errado"). O `tsconfig.json` aceita
 * JS na pasta (`allowJs`), o Vite empacota igual, e os tipos entram depois,
 * quando alguém tiver motivo para mexer aqui dentro.
 *
 * `geometria.ts` continua sendo a referência de como um módulo de domínio
 * TIPADO se parece. A diferença é o tamanho: 132 linhas contra 1.200.
 *
 * ---------------------------------------------------------------------------
 * A REGRA DA PASTA CONTINUA VALENDO
 * ---------------------------------------------------------------------------
 *
 * Zero `document`, zero `window`, zero canvas. Quem lê o arquivo e mede os
 * pixels é a tela; aqui entram só os bytes já lidos. É o que deixa este
 ...
 */
export function vetorizarImagem(dados, opcoes = {}) {
