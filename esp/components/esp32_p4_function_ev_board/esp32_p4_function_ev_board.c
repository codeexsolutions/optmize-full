...
esp_err_t bsp_display_brightness_init(void)
{
    ...
    const ledc_channel_config_t LCD_backlight_channel = {
        ...
        .duty = 0,
        .hpoint = 0,
        /* INVERTIDO: nesta placa a luz acende em nivel BAIXO. Sem isto,
         * "brilho 100%" apaga a tela -- que foi o que aconteceu o dia todo. */
...
#if !CONFIG_BSP_LCD_TYPE_HDMI
/*
 * ACRESCENTADO (nao esta no BSP original).
 *
 * O handle do toque era privado, e quem gira a tela precisa dele: o giro de
 * 180 graus acontece NO PAINEL, por comando dele, entao o controlador de toque
 * continua reportando coordenadas fisicas e passa a responder invertido. Quem
 * espelha o toque para acompanhar e o codigo da aplicacao -- e para isso ele
 * precisa alcancar o controlador.
 */
/*
 * ACRESCENTADO (nao esta no BSP original).
 *
 * O painel, para quem precisa falar com ele direto. O caso que motivou: girar a
 * tela 180 graus. Pelo LVGL (`lv_display_set_rotation`) o giro custa UM BUFFER
 * DE DESENHO INTEIRO -- 1,2 MB nesta tela --, porque ele rotaciona em software.
 * Pelo painel custa um comando: o EK79007 sabe espelhar sozinho.
 */
...
lv_display_t *bsp_display_start(void)
{
    bsp_display_cfg_t cfg = {
        ...
        .flags = {
            ...
            .buff_spiram = false,
            /*
             * DESLIGADO (o BSP original deixa `true`).
             *
             * Ligado, o `esp_lvgl_port` reserva um TERCEIRO buffer de desenho
             * inteiro -- 1,2 MB em 1024x600 -- para poder girar em software. E
             * ele reserva SEMPRE, mesmo que ninguem gire nada.
             *
             * Nesta placa a tela gira 180 graus no proprio painel
             * (`esp_lcd_panel_mirror`, ver `girar_de_cabeca_para_baixo` na
             * aplicacao), que custa um comando e nenhuma memoria. O buffer era
             * desperdicio puro -- e quando o leitor de QR entrou, foi ele que
             * faltou: `Not enough memory for LVGL buffer (rotation buffer)`,
             * com a placa reiniciando em laco antes de mostrar qualquer coisa.
             *
             * O preco de desligar: giros de 90 e 270 graus deixam de existir,
             * porque esses o painel nao faz sozinho. Se um dia forem precisos,
             * religa-se isto e paga-se a memoria.
             */
            .sw_rotate = false,
        }
    };
